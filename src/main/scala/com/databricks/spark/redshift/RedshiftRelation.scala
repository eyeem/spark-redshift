    ...
    if (requiredColumns.isEmpty) {
      ...
      val filesToRead: Seq[String] = {
        ...
        val s3Files = try {
          ...
          // Redshift changed its behaviour to not export any files when the table is empty. This catch block handles such scenario
          case e: AmazonS3Exception if e.getStatusCode == 404 => Seq()
        }
